...
class AddLesson extends BaseComponent {
  constructor(props, context) {
    ...
    this.state = {
      initialData: {},
      startTime: props.startTime || moment(new Date()),
    ...
    this._bind(
      ...
      'removeHourse',
      'selectStudent',
      'createStudentLessonList',
      'changeEnStatus',
      'changeHourse',
      'changePaidStatus',
      'updateStudentAssingStatus',
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      startTime: nextProps.calenderClicked
        ? nextProps.startTime
        : moment(new Date()),
      endTime: nextProps.calenderClicked
        ? nextProps.endTime
        : moment(new Date()),
      scheduledStartDate: nextProps.calenderClicked
        ? nextProps.scheduledStartDate
        : moment(new Date()),
      scheduledEndDate: nextProps.calenderClicked
        ? nextProps.scheduledEndDate
        : moment(new Date()),
  ...
  doSubmit(formData) {
    ...
    if (this.state.extraLessonDetail) {
      ...
      if (Object.keys(this.state.selectedHorses).length) {
        ...
        data['lesson_date_time'][
          'lesson_date_time_horses_attributes'
        ] = lessonHorse;
      }
  ...
  changeEnStatus(e, student) {
    ...
    var horse_id =
      data[student] && data[student]['horse_id']
        ? data[student]['horse_id']
        : null;
    var paid =
      data[student] && data[student]['paid'] ? data[student]['paid'] : false;
    data[student] = {
      enrollment_status_id: e.target.value,
      horse_id: horse_id,
      paid: paid
    };
    var lessonArray = this.updateStudentAssingStatus(
      student,
      'enrollment_status_id',
      e.target.value
    );
    this.setState({
      lessonStudentArray: data,
      lessonPeopleStates: lessonArray
    });
  }

  changeHourse(e, student) {
    var data = this.state.lessonStudentArray;
    var enrollment_status_id =
      data[student] && data[student]['enrollment_status_id']
        ? data[student]['enrollment_status_id']
        : null;
    var paid =
      data[student] && data[student]['paid'] ? data[student]['paid'] : false;
    data[student] = {
  ...
  createLocationList(location) {
    return (
      <div key={location.id} className="snglChck">
        <label>
          {location.location_name}
          <input
            checked={this.state.locationId == location.id}
            onChange={e => {
              this.changeLocation(e, location.id);
            }}
            type="radio"
            name="location"
          />
  ...
  createRepeatOptions(times) {
    return _.range(1, times).map(value => (
  ...
  dayCheckBoxes(times) {
    return _.range(0, times).map((value, key) => (
      <div key={key} className="cbSngl">
        <label>
          <p>{RepeatDays[key]}</p>
          <input
            type="checkbox"
  ...
  lessonHorseList(horse) {
    return (
      <div className="snglClDtl" key={horse.id}>
        <div className="clntNmeImg">
          <div className="clientImg">
            <img src="assets/clientImage.png" alt="Client Image" />
          </div>
  ...
  createStudentLessonList(student) {
    ...
    var enrollmentStatuses = _.map(
      this.props.enrollmentStatuses,
      this.createenrollmentStatusesList
    );

    return (
      <div className="snglClDtl" key={student.id}>
        <div className="clntNmeImg">
          <div className="clientImg red_clnt" />
          <div className="clntName">{student.student_name}</div>
        </div>
        <div
          className="adhCross"
          onClick={e => {
            this.removeStudent(e, student.id);
          }}
        >
          <button />
        </div>
            ...
            <div className="slct_cnt dblSelect clearfix">
              <div className="snglFld">
                ...
                <select
                  ...
                  onChange={e => {
                    this.changeEnStatus(e, student.id);
                  }}
                >
              ...
              <div className="snglFld">
                ...
                <select
                  className="selectpicker"
                  onChange={e => {
                    this.changeHourse(e, student.id);
                  }}
                >
  ...
  render() {
    ...
    var sections = _.map(this.props.sections, data => {
      return this.createOptions(data, 'section_name');
    });
    var instructors = _.map(this.props.instructors, data => {
      return this.createOptions(data, 'instructor_name');
    });
    var horses = _.map(this.props.horses, this.createHorsesList);
    ...
    var lessonStudentList = _.map(
      this.state.lessonStudents,
      this.createStudentLessonList
    );
              ...
              <div className="snglFld">
                ...
                <div className="snglChck">
                  <label>
                    NO<input
                      type="radio"
                      checked={this.state.isRecuring == false}
                      name="rcrng"
                      onChange={e => {
                        this.changeRecuring(e, false);
                      }}
                    />
                    <span />
                  </label>
                ...
                <div className="snglChck">
                  <label>
                    YES<input
                      type="radio"
                      checked={this.state.isRecuring == true}
                      name="rcrng"
                      onChange={e => {
                        this.changeRecuring(e, true);
                      }}
                    />
                    <span />
                  </label>
                ...
                <div className="rcrngYes">
                  <div className="slct_cnt dblSelect clearfix">
                    ...
                    <div className="snglFld">
                      <select
                        ...
                        className="selectpicker selectpickerRecuring"
                        onChange={e => {
                          this.selectRepeat(e);
                        }}
                      >
                  ...
                  {this.state.period == 'week' && (
                    <div className="slct_cnt dblSelect clearfix">
                      ...
                      <div className="chkbxCnt">{this.dayCheckBoxes(7)}</div>
                    </div>
                  )}
                  <div className="slct_cnt dblSelect clearfix">
                    ...
                    <div className="snglFld">
                      <DatePicker
                        ...
                        onChange={this.changeRepeatStartDate}
                        dateFormat="MMMM DD , YYYY"
                        readOnly={true}
                      />
                    </div>
                  ...
                  <div className="rcrngRdo">
                        ...
                        <div className="aftrOcnc">
                          ...
                          {this.state.repeatEnd == 'after' && (
                            <div className="aoInpt">
                              <input
                                type="text"
                                value={this.state.occurenceVal}
                                onChange={e => {
                                  this.endRepeatOccurance(e);
                                }}
                              />
                            </div>
                          )}
                      ...
                      <div className="snglChck">
                        <label>
                          <input
                            ...
                            checked={this.state.repeatEnd == 'on'}
                          />
                          <span />
                        </label>
                        <div className="aftrOcnc">
                          ...
                          <div className="slct_cnt">
                            <div className="snglFld">
                              {this.state.repeatEnd == 'on' && (
                                <DatePicker
                                  selected={this.state.endRepeatDate}
                                  onChange={this.changeEndRepeatDate}
                                  dateFormat="MMMM DD , YYYY"
                                  minDate={this.state.endRepeatMinDate}
                                  readOnly={true}
                                />
                              )}
                            </div>
                    ...
                    <div className="snglFld">
                      <label>
                        Current students and horses also should be copied ?
                      </label>
                      <div className="snglChck">
                        <label>
                          NO<input
                            type="radio"
                            checked={this.state.studentCopied == false}
                            name="stucp"
                            onChange={e => {
                              this.shouldStudentCopied(e, false);
                            }}
                          />
                          <span />
                        </label>
                      ...
                      <div className="snglChck">
                        <label>
                          YES<input
                            type="radio"
                            checked={this.state.studentCopied == true}
                            name="stucp"
                            onChange={e => {
                              this.shouldStudentCopied(e, true);
                            }}
                          />
                          <span />
                        </label>
              ...
              <div className="snglFld">
                      ...
                      <div className="popBody">
                        <div className="slct_cnt">
                          <div className="snglFld">
                            ...
                            <select
                              className="selectpicker selectpickerLessonHorse"
                              multiple
                              onChange={e => this.addHorseToLesson(e)}
                            >
                              {horses}
                            </select>
                          </div>
                        ...
                        <div className="addedHrse">
                          <div className="adhInr">{lessonHorses}</div>
                        </div>
                      </div>
                    </div>
                    <div className="snglBlck">
                      ...
                      <div className="popBody">
                        <div className="slct_cnt">
                          <div className="snglFld">
                            ...
                            <select
                              className="selectpicker selectpickerStudents"
                              multiple
                              onChange={e => {
                        ...
                        <div className="addedHrse">
                          <div className="adhInr">{lessonStudentList}</div>
                        </div>
                      </div>
                    </div>
                    <div className="snglBlck">
                      ...
                      <div className="popBody">
                        <div className="lnText">
                          <label>
                            Add a note visible to client and staff assigned to
                            this lesson (optional)
                          </label>
                          <textarea rows="6" ref="lesson_notes" />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="btmBtns">
                  ...
                  <div className="bbsnglBtn orangeBtn">
                    <button type="submit">
                      Add lesson and fill in details later
                    </button>
                  </div>
